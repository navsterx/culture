  ...
  <div class="p-home">
    <v-sheet color="primary" class="py-4">
      <v-container>
        ...
        <div class="text-body-1 text-white text-center mb-6">
          Search for a role and/or filter on the benefits & perks to see what
    ...
    <v-fade-transition hide-on-leave>
      <v-container v-if="isLoaded" key="loaded-content">
        <div
          class="text-center pa-8 text-body-1"
          v-if="filteredCompanies.length === 0"
        >
          Sorry, no companies were found that match your criteria, please try
          again
        </div>
...
useSeoMeta({
  title: `Awesome Culture - Discover UK tech companies that align with you`,
  description:
    "Easily filter companies based on the perks they offer, delve into detailed information about their culture, day-to-day operations, and training opportunities. The goal is to provide the insights you need to make informed decisions when searching for your next job.",
  ogDescription:
    "Easily filter companies based on the perks they offer, delve into detailed information about their culture, day- to - day operations, and training opportunities.The goal is to provide the insights you need to make informed decisions when searching for your next job.",
...
async function getCompanies() {
  try {
    const { data: data } = await supabase.rpc("getCompanies");
    companies.value = data.sort(
